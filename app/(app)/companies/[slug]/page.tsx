    ...
    <div className="grid grid-cols-[250px_1fr] min-h-screen">
      ...
      <main className="p-8 max-w-4xl mx-auto w-full">
        ...
        <p className="text-sm text-ink-dim mb-6">
          Written test patterns and focused strategy.
        </p>
        ...
        <div className="grid grid-cols-4 gap-3 mb-8">
          {(allCompanies ?? []).map((c) => (
            <a
              ...
              className={`glass-card p-4 text-center block ${
                c.slug === slug ? "border-vio" : ""
              }`}
            >
        ...
        <h2 className="text-base font-semibold mb-3">
          {company.name} — Written Test Pattern
        </h2>
        ...
        <div className="glass-card p-5 overflow-x-auto">
          <table className="w-full text-[12.5px] border-collapse">
            ...
            <tbody>
              {(company.sections as Section[]).map((s, i) => (
                <tr key={i}>
                  <td className="py-3 border-b border-glass-brd">
                    {s.name}
                  </td>
                  ...
                  <td className="py-3 border-b border-glass-brd capitalize">
                    {s.difficulty}
                  </td>
                </tr>
              ))}
          ...
          <h4 className="text-[13px] font-semibold mt-5 mb-2">
            Frequently Asked Topics
          </h4>

          <div className="flex gap-2 flex-wrap">
            {(company.focus_topics as string[]).map((t, i) => (
              <span
                key={i}
                className="text-[11px] border border-glass-brd rounded-full px-3 py-1"
              >
                {t}
              </span>
            ))}
          ...
          <p className="text-sm text-ink-dim leading-7">
            {company.strategy}
          </p>
        </div>
      </main>
