...
const badgeMap: Record<string, [string, string]> = {
  ...
  not_started: [
    "bg-glass text-ink-faint border border-glass-brd",
    "Start",
  ],
};
...
export default async function RoadmapPage() {
  ...
  const [{ data: subjects }, { data: topics }, { data: progress }] =
    await Promise.all([
      supabase.from("subjects").select("*").order("sort_order"),
      supabase
        .from("topics")
        .select("id, slug, name, subject_id, group_name, sort_order")
        .order("sort_order"),
      supabase
        .from("progress")
            ...
            <div key={subject.id} id={subject.slug} className="mb-9">
              <div className="flex items-center gap-2.5 mb-4">
                ...
                <span className="text-[11px] font-mono text-ink-faint">
                  {doneCount}/{subjectTopics.length} done
                </span>
              </div>
              ...
              <div className="relative pl-6 before:content-[''] before:absolute before:left-[9px] before:top-1.5 before:bottom-1.5 before:w-0.5 before:bg-gradient-to-b before:from-indig before:to-vio">
                {subjectTopics.map((topic) => {
                  const status =
                    progressMap.get(topic.id) ?? "not_started";
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between px-3.5 py-2">
                        ...
                        <span
                          className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full uppercase ${badgeCls}`}
                        >
                          {label}
                        </span>
                      </div>
